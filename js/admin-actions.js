...
async function loadMaterial(index) {
  ...
  if(currentMaterialType === 'exam_year') ui.btnAddSubject.textContent = '＋年度を追加';
  else if(currentMaterialType === 'exam_univ') ui.btnAddSubject.textContent = '＋大学を追加';
...
function handleAddSubject() {
    ...
    if(folderName && explanationsDirHandle) {
        getMaterialDirHandle().then(d => d.getDirectoryHandle(folderName, {create:true})).catch(console.warn);
    }
    renderTree();
    saveAll();
}
...
function setupImportModalEvents() {
    if(ui.btnCloseImport) ui.btnCloseImport.onclick = () => ui.importModal.style.display = 'none';
...
function createDynamicImportModal() {
    ...
    modalContent.querySelector('#btn-cancel-import-dyn').onclick = () => document.body.removeChild(modalOverlay);
    modalContent.querySelector('#btn-exec-smart-import-dyn').onclick = async () => {
...
async function executeSmartImport(htmlRaw, jsonRaw) {
  ...
  let metaData;
  try { metaData = JSON.parse(jsonClean); } 
  ...
  if (existingProbIndex !== -1) {
    if(!confirm(`問題ID "${metaData.id}" (タイトル: ${targetField.problems[existingProbIndex].title}) を上書きしますか？`)) return;
  ...
  if (htmlClean) {
    try {
      ...
      await w.close();
    } catch(e) { console.warn("HTML書き込みエラー", e); }
  }
