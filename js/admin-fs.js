...
async function getMaterialDirHandle() {
  ...
  return await explanationsDirHandle.getDirectoryHandle(matId, {
    create: true,
  });
}
...
async function fsRenameFolder(parentHandle, oldName, newName) {
  ...
  try {
    ...
    const newDir = await parentHandle.getDirectoryHandle(newName, {
      create: true,
    });
    ...
    for await (const [name, handle] of oldDir.entries()) {
      if (handle.kind === "file") {
        ...
        const newFileHandle = await newDir.getFileHandle(name, {
          create: true,
        });
...
async function fsMoveFile(currentPath, targetFolderHandle, newFileName) {
  try {
    ...
    const newHandle = await targetFolderHandle.getFileHandle(
