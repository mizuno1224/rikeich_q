  ...
  currentMaterialData.subjects.forEach((sub, sIdx) => {
    ...
    sub.fields.forEach((fld, fIdx) => {
      ...
      fld.problems.forEach((prob, pIdx) => {
        ...
        pDiv.addEventListener('dragleave', () => pDiv.style.borderTop = 'transparent');
...
async function openEditor(problem) {
  ...
  const updateJson = () => { 
      if(document.getElementById('json-editor-area')) 
          document.getElementById('json-editor-area').value = JSON.stringify(problem, null, 2); 
  };

  row1.appendChild(createInput('ID', problem.id, val => { 
      problem.id = val; 
  ...
  const titleGroup = createInput('タイトル', problem.title, val => { 
      ...
      const activeItem = ui.treeRoot.querySelector('.prob-item.active span:first-child');
